# Olá, eu sou Cauã 

Sou engenheiro de software com foco em **automações**, **integrações**, **IA aplicada a negócios** e **soluções que realmente resolvem problemas do dia a dia**.

Hoje, trabalho principalmente com projetos que envolvem:
- **HubSpot**
- **n8n**
- **APIs e integrações**
- **Agentes de IA**
- **WhatsApp + CRM**
- **WordPress / HubSpot CMS**
- **Soluções sob medida para operações, marketing e vendas**

Gosto de transformar processos manuais em fluxos inteligentes, criar sistemas mais eficientes e usar IA de forma prática no desenvolvimento.

---

## Sobre mim

Tenho experiência construindo projetos como:

- integrações entre plataformas como **HubSpot, Asaas, Omie, WhatsApp e APIs externas**
- automações de processos comerciais e operacionais
- chatbots e agentes de IA para atendimento, análise e produtividade
- dashboards, fluxos e sistemas internos para empresas
- sites e estruturas digitais com foco em performance, SEO e conversão

Meu foco não é só fazer funcionar.  
Eu gosto de entender a lógica, estruturar bem a solução e construir algo que seja escalável, claro e útil.

---

## Tecnologias e ferramentas

### Backend / Automação
![Python](https://img.shields.io/badge/Python-111?style=for-the-badge&logo=python)
![Node.js](https://img.shields.io/badge/Node.js-111?style=for-the-badge&logo=node.js)
![TypeScript](https://img.shields.io/badge/TypeScript-111?style=for-the-badge&logo=typescript)
![JavaScript](https://img.shields.io/badge/JavaScript-111?style=for-the-badge&logo=javascript)
![n8n](https://img.shields.io/badge/n8n-111?style=for-the-badge&logo=n8n)
![REST API](https://img.shields.io/badge/API_REST-111?style=for-the-badge)

### CRM / CMS / Plataformas
![HubSpot](https://img.shields.io/badge/HubSpot-111?style=for-the-badge&logo=hubspot)

### Frontend
![React](https://img.shields.io/badge/React-111?style=for-the-badge&logo=react)
![HTML5](https://img.shields.io/badge/HTML5-111?style=for-the-badge&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-111?style=for-the-badge&logo=css3)

### Banco de dados e outros
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-111?style=for-the-badge&logo=postgresql)
![MySQL](https://img.shields.io/badge/MySQL-111?style=for-the-badge&logo=mysql)
![Git](https://img.shields.io/badge/Git-111?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-111?style=for-the-badge&logo=github)

---

## No que eu gosto de trabalhar

- automações inteligentes
- integrações entre sistemas
- IA aplicada ao fluxo de trabalho
- arquitetura de lógica para chatbots e agentes
- soluções para CRM e operação comercial
- melhoria de processos com tecnologia

---

## Alguns temas que você vai encontrar por aqui

- projetos de integração com APIs
- automações com n8n
- soluções com HubSpot
- testes com agentes de IA
- estruturas full stack
- experimentos para produtividade e negócios

---

## Como falar comigo

- **LinkedIn:** https://www.linkedin.com/in/cau%C3%A3-tardetti-b031b9212/
- **E-mail:** cauabtvendrame@gmail.com

---

## GitHub Stats

<p align="center">
  <img src="./profile/stats.svg" alt="GitHub Stats" height="170" />
  <img src="./profile/top-langs.svg" alt="Top Languages" height="170" />
</p>

---

## Frase que resume meu jeito de construir

**Tecnologia boa é a que resolve, simplifica e escala.**
